...
For JSON event pipelines, the same work starts one stage earlier: [streaming large JSON in Python](/blog/large-json-python/parser-speed-is-not-the-whole-problem) keeps memory stable before records reach the database writer.
...
That is the write-side version of measuring the real constraint. In the [PgBouncer multicore benchmark](/blog/pgbouncer-multicore-limit/pgbouncer-from-45k-to-210k-queries-per-second), proxy CPU, not PostgreSQL capacity, was the first bottleneck.
