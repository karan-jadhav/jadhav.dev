...
The limiting layer can be different on the write path. In the [PostgreSQL bulk-ingestion benchmark](/blog/postgres-copy-ingest/1m-rows-into-postgres-in-1-29-seconds), client round trips and write shape—not raw database capacity—dominated the slow result.
