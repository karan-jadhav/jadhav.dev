...
seoTitle: "Python JSON: Memory Use vs Parser Speed"
...
That gap between compact input and runtime memory also appears in the [100M-vector TurboVec benchmark](/blog/turbovec-rag-vectors/turbovec-on-100m-rag-vectors), where the persisted index and prepared query process had very different footprints.
...
The same shape-first principle applies after parsing: the [PostgreSQL bulk-ingestion benchmark](/blog/postgres-copy-ingest/1m-rows-into-postgres-in-1-29-seconds) shows how batching and `COPY` change the write path before database tuning begins.
