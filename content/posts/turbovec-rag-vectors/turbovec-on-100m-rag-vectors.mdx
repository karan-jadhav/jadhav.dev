...
A smaller artifact does not guarantee a small runtime footprint. The [large-JSON Python benchmark](/blog/large-json-python/parser-speed-is-not-the-whole-problem) shows the same distinction from the ingestion side, where on-disk size stayed similar but object materialization changed peak memory by gigabytes.
